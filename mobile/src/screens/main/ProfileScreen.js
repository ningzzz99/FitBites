...
export default function ProfileScreen() {
  ...
  async function confirmUnlock(type, value) {
    const cost = type === 'color' ? 100 : 200;
    const currentCoins = data?.profile?.total_coins ?? 0;

    if (currentCoins < cost) {
      Alert.alert('Not enough coins', `You need ${cost} coins to unlock this ${type}. You currently have ${currentCoins} coins.`);
      return;
    }

    Alert.alert(
      'Unlock Item',
      `Unlock this ${type} for ${cost} coins?`,
      [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Unlock', onPress: () => handleUnlock(type, value) },
      ]
    );
  }
          ...
          {BANNER_COLORS.map((color) => {
            ...
            return (
              <Pressable
                ...
                onPress={() => (unlocked ? setBannerColor(color) : confirmUnlock('color', color))}
                disabled={busy}
              >
          ...
          {BANNER_ICONS.map((icon) => {
            ...
            return (
              <Pressable
                ...
                onPress={() => (unlocked ? setBannerIcon(icon.key) : confirmUnlock('icon', icon.key))}
                disabled={busy}
              >
                {unlocked ? (
